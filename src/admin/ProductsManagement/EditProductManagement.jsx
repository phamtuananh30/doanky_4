...
export default function EditProductManagement() {
  ...
  const {
    ...
    error: error2,
  } = useSwr("product/get/" + id, (endpoint) =>
    apiClient.get(endpoint).then((data) => data.data)
  );
  ...
  const handleCategoryToggle = (categoryId, categoryName) => {
    if (
      productData?.categories?.some((category) => category.id === categoryId)
    ) {
      formRef.current = {
        ...formRef.current,
        categories: formRef.current.categories.filter(
